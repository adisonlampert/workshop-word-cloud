...
`sample` is a string that contains Virginia Woolf's short story "A Haunted House" which we'll use to test our word cloud.
...
`anychart.tagCloud()` creates a Word Cloud graph object that we can fill with data using the `.data` function. The `.data` function takes in two parameters: the text to use as data and a dictionary of key-value pairs that dictate how to parse and display the data.
